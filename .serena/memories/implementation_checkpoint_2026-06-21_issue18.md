...
実装完了。ブランチ `worktree-issue-18-perf-budget`。型検査・単体テスト（727件、うち本Issueの判定器テスト11件）・通常ビルド（診断ページ含む）・本番ビルド（診断ページ `perf-budget.html` 除外確認）・スモーク（perf/layer/glow/screens/rendering）すべて成功。
...
- 既存スモーク（rendering/layer/glow/screens）回帰なし。通常構成で診断アクセサ未公開を確認（本番では FPS 計測フックを露出しない）。
...
## 提出状況
未コミット。ブランチ `worktree-issue-18-perf-budget` に変更がある状態。コミット・PRはユーザー指示を待つ。
