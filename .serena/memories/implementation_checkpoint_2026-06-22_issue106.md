...
**状態: Issue #106（舞台土台モデルの素材源・出典・規約適合の最終確定）の文言・記録の確定を実装し、PR #164 を作成・push 済み。ブランチ `worktree-issue-106-stage-compliance`。機能コード（ローダー・幾何生成・glb）は変更せず、文言と記録のみ確定。型検査・単体テスト951件・ビルド（Node 22）・保留文言のグレップ確認・定型句確認・glbのテクスチャ非同梱確認すべて通過。**
...
- 計画段階で CodeX 1回・Plan エージェント6回の二重チェックを反映（素材経路を3D機能と確定、テクスチャ非同梱の明記、公共データ利用規約による再配布許諾、保留文言の網羅範囲拡大、表記揺れの解消）。最終の着手判定は「着手可」。
- `npm run typecheck`（厳格・両設定）: 型エラーなし。
- `npm test`（vitest）: 951テスト全通過（`readmeConsistency.test.ts` の出典整合17件を含む）。
- ビルド `npm run build`（Node 22）: 通過。
- 保留文言の取りこぼし確認: 保留語と舞台土台を指す名詞でグレップし、舞台土台の能動的保留文言は残存なし（残る一致はミクのモーション別件・ロードマップ図・確定済み参照・内部状態の別意味のみ）。
- 提出 glb のバイナリ確認: 画像・テクスチャ参照ゼロ（頂点形状のみ）。

## 次の主要作業

1. PR #164 のマージ（ゴール基準の再レビューを経て）。
