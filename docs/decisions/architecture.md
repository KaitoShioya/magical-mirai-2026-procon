...
| 読ませる歌詞 | キネティックタイポグラフィ（troika-three-text） | 演出文字と同じWebGL描画で歌詞も描き、可読性は発光抑制・縁取り・影で担保する（後続Issueで導入） |
...
| `src/typography/` | 文字に関わる層を `kineticText/`（troika-three-textによる3次元の文字エンジン）に集約する。演出の文字も読ませる歌詞も同じエンジンが描き、可読性処理（発光抑制・縁取り・影・最小表示寸法）を内包する | マイルストーンM3（文字エンジンは #20、可読性処理は #31） |
