 ...
 * 例: /prototype.html?refl=256&bloomScale=0.4&points=400
 */
...
if (BLOOM) {
  ...
  composer.addPass(
    new UnrealBloomPass(new THREE.Vector2(w, h), 1.2, 0.6, 0.5)
  );
}
...
function animate() {
  ...
  if (now - last >= 500) {
    ...
    hud.textContent =
      `FPS ${fps}  avg ${window.__avgFps().toFixed(0)}\n` +
