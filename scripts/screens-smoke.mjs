...
try {
  ...
  // 1.5 曲選択UIの確認（Issue #5・横展開 Issue #91）。実装済み曲だけが開始でき、未実装曲は無効化されている。
  //     実装済みは TAKEOVER とアフター・ザ・カーテンとトリツクロジーの3曲のため、開始ボタンはちょうど3個（data-song-key が3曲）、
  //     準備中の無効ボタンが3個あることを機械的に確認する。
  const EXPECTED_STARTABLE_KEYS = ["after-the-curtain", "takeover", "toritsuku-logy"];
  ...
  if (songSelection.startCount !== EXPECTED_STARTABLE_KEYS.length) {
    ...
    console.log("確認: 開始できる曲は TAKEOVER とアフター・ザ・カーテンとトリツクロジーの3曲");
  }
