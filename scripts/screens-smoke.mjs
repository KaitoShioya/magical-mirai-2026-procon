...
try {
  ...
  //     実装済みは TAKEOVER とアフター・ザ・カーテンの2曲のため、開始ボタンはちょうど2個（data-song-key が両曲）、
  //     準備中の無効ボタンが4個あることを機械的に確認する。
  const EXPECTED_STARTABLE_KEYS = ["after-the-curtain", "takeover"];
  ...
  if (songSelection.startCount !== EXPECTED_STARTABLE_KEYS.length) {
    ...
    console.log("確認: 開始できる曲は TAKEOVER とアフター・ザ・カーテンの2曲");
  }
