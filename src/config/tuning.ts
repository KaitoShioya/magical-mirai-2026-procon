...
// --- 持続配置の灯し（タップ時に置く蝶のカメラ前方オフセット） ---
// 得点が出たタップごとに、カメラ軌跡上の通過点（カメラ位置）へ蝶を持続配置する。これは描画内部の定数ではなく、
// プレイ進行（src/app/playSession）が読むタップ配置の調整値である。カメラ軌跡を持つ playSession が配置点を算出し、
// 描画層（rendering）には算出済みの座標だけを渡す（描画層をカメラ軌跡へ依存させないため）。もし将来この値を描画層
// （renderRoot）の内部だけで使うことが判明したら、src/rendering/constants.ts へ移す。
// 出典: docs/idea/concept-final.md §5（カメラ軌跡が灯しの配置を決める）、本タスクのユーザー決定。
// 消費Issue: 本タスク（タップ時の持続配置）。

/**
 * 持続配置の蝶を、カメラ位置から視線方向（カメラ位置→注視点の単位ベクトル）へ前方へずらす距離（ワールド単位）。
 * 採用理由を先に述べる。カメラ位置そのものへ蝶を置くと、配置した瞬間はカメラの近傍（近接面0.1の内側）に入り、
 * クリップまたは過大表示で不自然になる。これを避けるため小さく前方へずらす。近接面0.1の10倍にあたり、最小の蝶
 * （半径相当0.5）がカメラ前方で完全に近接面の外へ出る一方、湖の規模（半径およそ28）に対しては十分小さく「軌道上の
 * 通過点に置く」意味を保てる距離として1.0を初期値とする。美観依存値のため実機目視で微調整する。★暫定。
 */
export const LANTERN_BUTTERFLY_FORWARD_OFFSET = 1.0;

// --- 持続配置のひまわりの放射状リング配置 ---
// ひまわりは蝶（カメラ通過点）とは独立に、初音ミク（湖の中心＝原点、src/config/character.ts の position）を中心とする
// 同心円状の領域へ配置する。反応が正確なほど中心に近い半径へ置き、半径帯ごとに自然な密度の上限を設け、満杯のときは
// その外側の最も近い帯へ散らして置く。これは描画内部の値でなく、playSession（app層）が配置位置を算出するための調整値で
// ある。世界単位の目安は 1メートル≒3.68ワールド単位（初音ミク159cm≒5.85ワールド単位）。
// 出典: 本タスクのユーザー決定（実機目視のフィードバック）。消費Issue: 本タスク。

/**
 * 反応が最も正確なときのひまわりの中心（ミク）からの半径（ワールド単位）。採用理由を先に述べる。中心のミク（足元の占有
 * は数単位）と重ならず、かつ最も正確な反応の灯しがミクの近くに集まる距離として、約0.8メートルにあたる3.0を採る。★暫定。
 */
export const LANTERN_SUNFLOWER_RADIUS_MIN = 3.0;

/**
 * 反応が最も不正確なときのひまわりの半径（ワールド単位）。採用理由を先に述べる。灯しの及ぶ半径は約28ワールド単位
 * （描画定数の根拠）であり、その内側に収まる約7メートルにあたる26.0を採る。★暫定。
 */
export const LANTERN_SUNFLOWER_RADIUS_MAX = 26.0;

/**
 * 半径帯（同心円のバンド）の数。採用理由を先に述べる。最小半径から最大半径までを等幅で分け、反応精度の段階に対応づける。
 * 7段にすると1帯の幅が約3.3ワールド単位（約0.9メートル）となり、精度差が半径差として知覚できる粒度になる。★暫定。
 */
export const LANTERN_SUNFLOWER_RING_COUNT = 7;

/**
 * ひまわり1個が占める面積の一辺（ワールド単位。中心間隔の目安）。採用理由を先に述べる。各帯（円環領域）の収容上限を
 * 「帯の面積 ÷ この値の2乗」で決めるための値で、小さいほど密に敷き詰められる。ひまわりの花直径は約0.74ワールド単位で、
 * 花どうしが近接しつつ過度に重ならない中心間隔として、その約1.35倍にあたる1.0を採る。これにより円環領域を密に充填でき、
 * 全帯の合計収容数は得点タップの最大数を十分に上回る（中心寄りの帯から密に埋まり、外側へはあふれた分だけ広がる）。★暫定。
 */
export const LANTERN_SUNFLOWER_MIN_SPACING = 1.0;
