// 依存規則（docs/decisions/architecture.md §5、src/input/README.md）の静的検査。
// 入力の各ソースが禁止依存（profiles・tools・rendering の各ディレクトリと three.js 本体）を参照しないことを、
// 実装時に機械的に保証する。本文全体の文字列検索では注釈や識別子名の偶然の一致で誤検知するおそれがあるため、
// 依存先を指す指定子だけを取り出して経路で判定する。
...
const srcDir = resolve(inputDir, "..");

/** 入力ディレクトリ配下の、テストでない TypeScript ソースの絶対パスを集める。 */
...
/** 指定子が禁止依存を指すかを判定する。three 本体と、profiles・tools・rendering 配下を違反とする。 */
function isForbidden(specifier: string, fromFile: string): boolean {
  ...
  if (specifier.startsWith(".")) {
    ...
    const forbiddenRoots = ["profiles", "tools", "rendering"].map((name) =>
...
describe("入力の依存境界", () => {
  const sources = collectSourceFiles(inputDir);

  it("検査対象のソースが1つ以上ある", () => {
    expect(sources.length).toBeGreaterThan(0);
  });

  it("禁止依存（profiles・tools・rendering・three.js）を参照しない", () => {
