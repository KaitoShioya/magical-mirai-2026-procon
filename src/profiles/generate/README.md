...
- **責務**: 楽曲解析データ（拍・サビ区間・歌詞文字の開始時刻・見せ場・クライマックス代表時刻）から、曲全体を時間方向に切れ目なく覆う密度プランを決定論的に生成する。密度プランは中間データで曲プロファイルには保存しない（スキーマに密度フィールドは無い）。下流の Issue #38（オンセット選別）と Issue #44（タップ総数上限）が消費する。設計根拠は `docs/research/04-ux-and-chart-design.md` 第4節と `docs/research/07-feasibility-and-parameters.md` 第2.1節・第2.6節。
- **区間モデル**: サビ＝拍あたり0.75（譜面再設計で毎拍1.0から抑え、密度の波を強調選別で作る）、非サビ基本＝拍あたり0.5（2拍1回）、休符＝置かない、溜め＝見せ場区間の直前1小節の非サビ助走（拍あたり0.25）。分類の優先順位はサビ＞休符＞溜め＞基本。密度の値は0.25刻みに限定する（`countTargetNotes` が0.25で割った整数を累積するため）。物語弧の表現はこの密度の対比と休符・溜めで充足する。
- **境界と計数の分離**: 区間境界は音楽地図の時刻（サビ区間の開始・終端など）を正とし、拍がどの区間に属するかは拍の開始時刻で許容差なしの厳密な右半開比較で判定する。これにより各サビ64拍・骨格386が保たれる（第2サビ開始の生値は88800.20000000001で、直前拍88800.2は非サビになる）。
- **計数の2系統**: 骨格ノーツ数は分類別の合計式（サビ密度・基本密度のみ）で数え、Issue #44 の `tapBudget.fullPossible` の入力になる。実効目標ノーツ数は分類済み拍列を時間順に歩く整数累積（サビ3・基本2・溜め1・休符0、4到達で1ノーツ、端数を持ち越す）で数え、区間別割当も同時に得る。TAKEOVERは骨格386・実効339（休符・溜めの削減後）。実効の区間別割当 `byRegion.targetNotes` を Issue #38 のオンセット選別が区間別目標数として消費する。
- **量子化**: 最小間隔は、クライマックス見せ場の窓とサビの重なる区間で86ミリ秒（16分音符＝60000÷175÷4）、その他は171ミリ秒（8分音符）。同一スロットの連打の最小間隔は171ミリ秒。これらは密度プランの保有値で、再設計のオンセット選別は1拍1ノーツの上限を守るため拍間隔（約343ミリ秒）が常にこれらを上回り、最小間隔の違反は構造的に起きない。
...
- **責務（譜面再設計）**: 拍格子と密度プラン（#43 の区間分類・区間別目標数）と音楽的強調信号（小節内拍位置・コード変化・声量・歌詞オンセット・見せ場信号）から、各区間で強調スコア上位の拍を区間別目標数ぶん選び、中間ノーツの配列を返す。`docs/research/04-ux-and-chart-design.md` §4 のノーツ生成の第1段にあたる。出力は最終 `SongProfile.notes` の第1段で、`slotIndex`・`pattern`（#39）と `trajectoryPosition`（#40）は後段が付与するため中間型 `OnsetNote` には持たせない。
- **旧設計からの変更理由**: 旧実装は拍格子だけの機械間引き（サビ毎拍・非サビ2拍に1回）で、声量・コード変化・歌詞密度・小節内拍位置を読まず休符・溜め・配置の変化が無く単調だった。再設計は密度プランを実際にノーツ選別へ結線し、強調で「どの拍に置くか」を選び、休符を置かず溜めを疎にして配置を多様化する。
- **公開関数**: `generateOnsetNotes(input, options?) => OnsetNote[]` — 中間ノーツを拍索引昇順で返す。区間別目標数は #43 の `countTargetNotes(plan).byRegion.targetNotes` をそのまま使い密度設計と数値を一致させる。各区間で強調スコア降順に目標数だけ選び、同点は拍索引昇順で決定論にする。休符区間は何も置かない。
- **サビ反復の共有テンプレート**: サビ3反復は反復不変の信号（小節内拍位置とサビ開始基準の相対コード変化）だけから先頭サビで相対拍位置（beatOffset）を選び、同一の相対集合を各反復へ写す。これで3反復の beatOffset 集合が一致し、多様性逓減（`diversityIndex.ts`）の同リズム前提を満たす。全反復が同一拍数であることを前提とし、拍数が一致しない場合は文脈付き例外で失敗させる。
- **出口の重複検査**: 選別出口で同一 beatIndex のノーツが無いことを検査し、あれば文脈付き例外で失敗させる（多様性逓減のキー衝突と判定の同時刻化を発生源で止めるため）。
- **オプション既定値（`DEFAULT_ONSET_OPTIONS`）**: 強調スコアの重み（小節内強拍・コード変化近接・声量・歌詞オンセット近接・見せ場信号）と近接核半幅・id接頭辞。各値の採用理由は同ファイルの関数コメントに先に明記する。
- **依存の向き**: `engine` 等の中核から import されない。`tools`・`rendering`・three.js を import しない。`./types`（共通型 `ChorusSegment`）だけを取り込み、最終 `Note` 型にも依存しない。
- **後段との契約**: 後段（#39・#40・#45・#46）が `OnsetNote` から最終 `Note` を作るときは、`id`・`timeMs`・`beatIndex` だけを引き継ぎ、`slotIndex`（#39）・`pattern`（#39）・`trajectoryPosition`（#40）を付与する。中間メタデータの `sectionKind` は最終 `Note` の項目ではないため最終出力に含めない。
- **担当Issue**: #38（譜面再設計で全面改稿）。後続の #39（slotIndex・pattern 付与）・#40（trajectoryPosition 付与）・#45（生成スクリプト）・#46（TAKEOVERプロファイル生成）が本関数の出力を入力に使う。
...
- **割当方式（譜面再設計）**: 旋律はTAKEOVERで信頼性が低く使えないため（§2）、その瞬間の和音（コードトーン）の `slots[].pitches` を音高の土台にし、`slotIndex` 空間で次の4要素により番号を決める。第1に起点の音域分散で、フレーズ先頭ごとに循環序数（係数3はスロット数7と互いに素で全7スロットを巡る）から起点を再シードして中央偏りを防ぐ。第2に配置語彙（階段の昇降＝±1連続・トリル＝±1往復・跳躍＝差の絶対値2以上・意図的な同音）を、声量と小節内拍位置から決定論的に選んで `slotIndex` 列へ反映する。第3に跳躍を置く位置の優先順位で、フレーズ先頭・コード変化境界・小節内強拍の順に重み、件数はストライド間引きで番号変化の20〜45パーセントに収める。第4にサビ反復の役割変換で、主題の `slotIndex` 列を同一 beatOffset で変奏（音域帯を下へ移す）・回帰（音域を上へ広げる）へ決定論的に写し、3反復の番号を対比させる。最後に同一 `slotIndex` の連続が上限（既定2）を超えないよう最終列で抑える。
- **同時押し（2点から3点）は本モジュールでは扱わない**: 1オンセットを1ノーツに保ち、タップ総数の母数（#44）と整合させる。同時押しは下流（#46・#48・#55）へ分離する。
- **公開関数**:
  - `applyNotePatterns(input, options?) => PatternedNote[]` — 中間ノーツ `PatternedNote`（`id`・`timeMs`・`beatIndex`・`slotIndex`・`pattern`）を返す。本モジュールの主たる成果物。
- **`pattern` の決め方と前後基準の違い**: `pattern`（`"ascending"`・`"descending"`・`"sameTone"`）は確定後の `slotIndex` の差から決める。天井や床のクランプで `slotIndex` が動かない箇所が確実に同音連打になり、表示上のY移動と `pattern` が一致するためである。区間の先頭ノーツは同じ区間の直後ノーツとの差で性格付け（区間の長さが1または差が0なら同音連打）、それ以外のノーツは直前ノーツとの差で決める。前後の基準が異なる点に注意する。
- **後段との契約**: #40 は `PatternedNote` の `id`・`timeMs`・`beatIndex`・`slotIndex`・`pattern` を引き継ぎ、`trajectoryPosition` を付与して最終 `Note` にする。
- **依存の向き**: `engine` 等の中核から import されない。`tools`・`rendering`・three.js を import しない。`./onsetNotes`（`OnsetNote` 型）と `../schema/profileSchema`（`ChordToneSlotRegion`・`LoudnessCurve`・`DiversityZone` 型）だけを取り込み、最終 `Note` 型にも依存しない。
- **オプション既定値（`DEFAULT_NOTE_PATTERN_OPTIONS`）**: `maxRun=2`（同一 `slotIndex` の連続上限。四分音符グリッドで3連続は同一スロットの連打になり単調と疲労を生むため2に抑える）/ `loudThreshold=0.5`（大きい声量とみなす正規化声量の閾値。中点で大小を素直に分ける初期値）/ `leapStride=3`（コード境界・強拍の跳躍候補を3個に1個だけ跳躍させる間引き。TAKEOVERで跳躍が番号変化の2〜4割に収まる）。いずれも初期見積もりでプレイ検証で調整する。
- **担当Issue**: #39（譜面再設計で全面改稿）。後続の #40（`trajectoryPosition` 付与）・#45（生成スクリプト）・#46（TAKEOVERプロファイル生成）が本関数の出力を入力に使う。
