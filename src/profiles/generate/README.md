# profiles/generate — 曲プロファイル生成の純粋関数群（Issue #41・#37）
...
`showcases.takeover.test.ts` が `docs/analysis/takeover.songmap.json` を素読みして達成基準1〜5を実データで表明する（`src/tools/` を import しない）。各層の単体テストは同居の `*.test.ts`。
