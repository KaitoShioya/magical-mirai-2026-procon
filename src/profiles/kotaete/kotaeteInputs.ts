...
export const kotaeteInputs: ManualProfileInputs = {
  ...
  // サビ区間が9個の「こたえて」は自動で9個の見せ場（全サビ区間）になる（下の mergeContiguousChorus を偽にして各サビ区間を
  // 独立に数えるため。統合すると4ブロックになり9にならない）。
  ...
  // 連続サビ統合を無効化する。「こたえて」は9個のサビ区間をそれぞれ独立の見せ場にする（ユーザー決定: 譜面の見せ場は9個）。
  // 既定（true）では隣接サビ区間が4ブロックへ統合され見せ場6個（サビ群4＋非サビ2）になり、狭いサビ間隙（97〜107秒の1570ミリ秒・
  // 208〜210秒の1590ミリ秒）へ非サビ窓が落ちて下限2000ミリ秒を割り生成が失敗する。各サビ区間を独立窓にしても、最終サビ区間
  //（223240〜236050ミリ秒）を覆うクライマックス窓の延長は曲末までで隣接窓へ食い込まないため重ならない（kotaeteProfile.gate.test.ts が
  // 区間の重複0件を固定する）。
