// ランク専用ゲージ（Issue #65）の表示物。判定レーン（落下式レーン #57）の上に縦バーを立て、スコア蓄積の
...
// 採用理由を先に述べる。判定レーンは画面右下で帯の上端が 0.30、横位置が右端から余白0.08とレーン幅の半分0.09
// 内側にある。ゲージはその上端の上に、レーンと同じ横位置・同じ幅で重ね、「判定レーンの上」を成立させる。

/** ゲージ（トラック）の横幅。レーン幅と揃える。 */
const GAUGE_WIDTH = 0.18;
/** レーンと同じ右余白。横位置をレーンと一致させるために使う。 */
const GAUGE_RIGHT_MARGIN = 0.08;
/** トラック下端。レーン上端0.30の上に間隔0.04を空ける。 */
const TRACK_BOTTOM_Y = 0.34;
/** トラックの高さ。 */
const TRACK_HEIGHT = 0.5;
/** トラック上端（下端＋高さ）。 */
const TRACK_TOP_Y = TRACK_BOTTOM_Y + TRACK_HEIGHT; // 0.84
/** トラックの中心の縦位置。 */
const TRACK_CENTER_Y = (TRACK_BOTTOM_Y + TRACK_TOP_Y) / 2; // 0.59
/** ランク文字の中心の縦位置。トラック上端の上に置く。 */
const LETTER_CENTER_Y = 0.89;
...
/** 文字の単位高さの上限。採用理由: 中心 0.89 から上端が画面上端 +1 を越えないよう、上限を 2×(1−0.89)=0.22 未満の
 *  0.20 とし、縦の短い画面でも文字が切れないようにする（上端は 0.89+0.10=0.99 で +1 未満）。 */
