...
- **レーン選択精度の0と0.2は別物**: 床のタップ（対応ノーツ無し、または信頼できないフレーム）の `pitchAccuracy` は0であり、これは「比較する正解レーンが存在しない」ことを表す。対応ノーツが有って正解と別のレーンを叩いたタップの `pitchAccuracy` は床値0.2（`PITCH_MISS_FLOOR`）であり、これは「正解レーンは存在するが一致しなかった」ことを表す。#51 反応強度はこの2値をそのまま区別して明るさへ写す。
...
- **出力は2チャンネル独立**: `size`（大きさを駆動する0以上1以下の強度）はタイミング精度のみに由来し、`brightness`（明るさを駆動する0以上1以下の強度）はレーン選択精度のみに由来する（出典 `docs/idea/concept-final.md` §6「タイミング精度→大きさ、レーン選択精度→輝度」）。スコアへ寄与する単一の総合値はここでは作らない（#55・#56 が各精度とJUSTを直接消費する）。
- **写像は恒等**: 精度をそのまま強度として通す（丸めと非有限値の防御のみ施す）。受け入れ基準「精度が高いほど光点が大きく明るい」を単調増加で満たし、新しい閾値や係数を持ち込まない。
- **レーン選択精度の3状態をそのまま区別**: 床のタップ（レーン選択精度0）→明るさ強度0、別のレーンを叩いたタップ（レーン選択精度0.2）→明るさ強度0.2、レーン一致（レーン選択精度1.0）→明るさ強度1.0。床が最も暗く、外しレーンがやや明るく、一致が最も明るい階調になる。
...
- **素点 a は本層が合成する**: `tapBaseScore(judgment)` がタイミング精度とレーン選択精度から素点 a（0以上1以下）を作る。重みは等重み（各0.5）で本層が所有し tuning.ts に置かない。
