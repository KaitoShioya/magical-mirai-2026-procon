...
export interface Playback {
  ...
  play(): void;
  /**
   * 楽曲（背景音楽）の音量倍率を設定する（0以上100以下。0で無音、100で本アプリの基準再生音量）。設定画面の音量つまみ
   * （Issue #77）が、操作音と合わせたマスター音量として使う。基準再生音量（楽曲を操作音とのバランスのため控えめにした値）へ
   * この倍率を掛ける。範囲外は0以上100以下へ丸める。音漏れ防止のため無音化している間（許可確立・先頭移動の途中）は
   * 即時反映せず、再生音量へ戻す復元時にこの倍率を反映する。
   */
