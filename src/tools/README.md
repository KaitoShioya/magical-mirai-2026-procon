...
- **守る契約**: `analysis` は `window.__songMap`・`document.title="DUMP_READY"`・`#status` を維持（`scripts/dump-songmap.mjs`）。`perf` は `window.__fps`/`__avgFps`/`__resetFps` とクエリノブを維持（`scripts/prototype-fps.mjs`）。
- **担当Issue**: #1
